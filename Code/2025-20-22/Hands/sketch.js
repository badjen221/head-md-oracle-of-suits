...
function draw() {
  ...
  // dessinez des barres de couleur où on peut dimunuer ou augmenter leur taille avec l'index
   if (detections) {
    for (let hand of detections.multiHandLandmarks) {
      // Obtenir la position de l'index
      let indexTip = hand[FINGER_TIPS.index];
      let x = indexTip.x * videoElement.width;
      let y = indexTip.y * videoElement.height;

      // Dessiner et mettre à jour les barres
      noStroke();
      for (let bar of colorBars) {
        // Vérifier si l'index est proche de la barre
        if (abs(x - bar.x) < bar.width) {
          // Ajuster la hauteur en fonction de la position Y de l'index
          bar.height = height - y;
        }
